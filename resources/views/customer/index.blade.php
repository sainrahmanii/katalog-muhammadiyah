            ...
            <div class="border rounded-xl">
                ...
                <div class="mx-3 mb-3">
                    ...
                    <div class="flex items-center justify-between">
                        <h1 class="my-1 font-semibold text-slate-800 truncate hover:text-clip">{{ $product->detail }}</h1>
                        @if ($product->available == 1)
                            <span class="bg-green-400 rounded-full text-sm font-bold px-3 py-1">TERSEDIA</span>
                        @elseif ($product->available == 0)
                            <span class="bg-red-400 rounded-full text-sm font-bold px-3 py-1">TIDAK TERSEDIA</span>
                        @endif
                    </div>
                    @if ($product->available == 1)
                        <a href="{{ route('show', $product->id) }}">
                            <button class="bg-cyan-300 w-full mt-2 px-3 py-2 rounded-lg font-semibold">Rp
